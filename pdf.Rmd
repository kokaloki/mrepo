...
literacy_data <- read_excel(tf)

```


# Peak literacy rates across countries in 2018
...
literacy_data %>%
  ...
  group_by(Country) %>%
  filter(
    max(Year) == 2018 & n_distinct(Year) >= 5,
    mean(`Literacy rate`[Year == 2018], na.rm = T) > mean(`Literacy rate`[Year !=
                                                                            2018], na.rm = T)
  ) %>%
  group_by(Country, Year) %>%
  summarise(Average_literacy_rate = mean(`Literacy rate`, na.rm = T)) %>%
  ggplot(aes(
    reorder(Country, Average_literacy_rate, mean),
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Countries that had peak average literacy rate in <span style='color:firebrick'>**2018**</span>",
    color = NULL
  ) +
  theme_ipsum_ps() +
...
## Literacy rate distribution across countries
...
literacy_data %>%
  ...
  geom_text(
    aes(
      x = Region,
      y = region_avg,
      label = scales::percent(region_avg)
    ),
    color = "white",
    hjust = 0.5,
    nudge_y = 0.01
  ...
  annotate(
    ...
    x = 7,
    y = 1,
    label = paste0("Worldwide literacy average of\n", scales::percent(world_avg)),
    color = "white"
  ) +
  scale_y_percent() +
...
```{r}
avg_year <- literacy_data %>% 
  group_by(Year,Region,Gender) %>%
  summarise(avg=mean(`Literacy rate`,na.rm = T)) %>%
  ungroup()


avg_year %>% 
  ggplot(aes(Year,avg,color=Gender))+
  geom_line(size=1.2)+
  geom_point(size = 2.6, aes(color = Gender), shape = 15) +
  facet_wrap(~Region,scales = "free")+
  scale_color_manual(values = c("female"="#d20962","male"="#005670","total"="#ce181e"))+
  scale_y_percent()+
  labs(x=NULL,y=NULL,title = "Literacy rate trend across different regions",subtitle = "Split by gender")+
   theme(
    text = element_text(family = "Roboto Condensed"),
    plot.margin = unit(rep(1.2, 4), "cm"),
    plot.title = element_text(size = 20, 
    ...
    strip.text = element_text(colour = "white",family = "Roboto Condensed",face = "bold")
  )
```

# Countries where average female literacy rate is greater than average male literacy rate
